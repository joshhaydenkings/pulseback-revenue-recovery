...
const nav = [
  ["Overview", "/", Gauge],
  ["Recovery Queue", "/recoveries", CircleDollarSign],
  ["Leak Map", "/leaks", Activity],
  ["Recovery Lab", "/lab", FlaskConical],
  ["Policies", "/policies", ShieldCheck],
  ["Audit Trail", "/audit", BookOpenCheck],
...
export function AppShell({
  ...
  const changeMode = async (mode: OperatingMode) => {
    ...
    if (
      mode === "AUTOPILOT" &&
      !window.confirm(
        "Enable bounded autonomous recovery? Guardian limits will still apply.",
      )
    )
      return;
    ...
    <div className="app-frame">
      <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
        ...
        <div className="mode-card">
          ...
          >
            <i /> {policies?.operatingMode ?? "AUTOPILOT"}{" "}
            <ChevronDown size={13} />
          </button>
          {modeOpen && (
          ...
          <small>
            {policies?.storage === "postgresql"
              ...
              : "Demo fallback · Guardian protected"}
          </small>
        </div>
        <nav>
