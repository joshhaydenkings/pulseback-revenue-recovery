...
export function CustomerCommunication({ caseId, hasPaymentLink }: { caseId: string; hasPaymentLink: boolean }) {
  ...
  return <section className="panel communication-panel">
    ...
    <div className="communication-status"><span>Payment Link</span><b>{hasPaymentLink ? "READY" : "REQUIRED"}</b></div>
    <p className="communication-copy">Preview the exact template before a Guardian-checked email leaves PulseBack.</p>
    <div className="communication-actions">
      <button className="secondary-button" onClick={loadPreview} disabled={Boolean(pending) || !hasPaymentLink}>{pending === "preview" ? <Loader2 className="spin" size={13} /> : <Eye size={13} />} Preview email</button>
      <button className="primary-button" onClick={send} disabled={Boolean(pending) || !hasPaymentLink}>{pending === "send" ? <Loader2 className="spin" size={13} /> : <Mail size={13} />} Send recovery email</button>
    </div>
    ...
    {preview && <div className="email-preview-backdrop" role="dialog" aria-modal="true" aria-label="Recovery email preview"><div className="email-preview-modal">
      <div className="email-preview-head"><div><span>TO {preview.recipient}</span><h3>{preview.subject}</h3></div><button aria-label="Close email preview" onClick={() => setPreview(undefined)}><X size={16} /></button></div>
