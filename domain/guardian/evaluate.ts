...
export function evaluateGuardian(recovery: Pick<RecoveryCase,'amountPaise'|'attempts'|'riskFlags'|'memory'|'activePaymentLinkId'|'operatingMode'>, ai: RecoveryDecision, policies: GuardianPolicies): GuardianResult {
  const contactLimitExceeded = recovery.memory.contacts24h >= policies.contactsPer24h || recovery.memory.contacts7d >= policies.contactsPer7d;
  const rules = [
  ...
  if ((policies.highRiskAutoStop && recovery.riskFlags.length) || contactLimitExceeded || recovery.memory.fatigueScore>=policies.fatigueStopThreshold || recovery.attempts>=policies.maxAttemptsPerCase || (ai.recommendedAction==='CREATE_PAYMENT_LINK' && recovery.activePaymentLinkId)) return {decision:'BLOCKED',reasons,rules};
